...
int main() {
  ...
  try {
    ...
    if (properties.major != 10 || properties.minor != 0) {
      std::cout << "SKIP: 2-SM tcgen05/TMA multicast test requires SM100a\n";
      return 0;
    }
    ...
    for (std::size_t i = 0; i < host_a.size(); ++i) {
      host_a[i] = Bf16(static_cast<float>(static_cast<int>(i % 7) - 3) /
                       16.0f);
    }
    for (std::size_t i = 0; i < host_b.size(); ++i) {
      host_b[i] = Bf16(static_cast<float>(static_cast<int>((i * 5) % 13) - 6) /
                       16.0f);
    }
    ...
    status = plan.run();
    require(status == KernelStatus::kSuccess, kernel_status_string(status));
    require_cuda(cudaDeviceSynchronize(), "2-SM GEMM execution");
    ...
    for (std::uint32_t row = 0; row < kM; ++row) {
      for (std::uint32_t column = 0; column < kN; ++column) {
        ...
        require(absolute_error <= 0.04f + 0.02f * std::abs(reference),
                "2-SM BF16 GEMM result exceeds tolerance");
      }
    }

    const auto stats = plan.stats();
    require(stats.tile_m == 256 && stats.tile_n == 128 && stats.tile_k == 64,
            "unexpected 2-SM MMA tile shape");
    std::cout << "SM100 2-SM BF16 GEMM correctness passed"
              << " (device=" << properties.name
              << ", kernel=" << sm100_2sm_kernel_name()
              << ", max_abs_error=" << max_absolute_error
              << ", max_rel_error=" << max_relative_error << ")\n";
    return 0;
  } catch (const std::exception& error) {
    std::cerr << "SM100 2-SM correctness test failed: " << error.what()
              << '\n';
    return 1;
  }
}
