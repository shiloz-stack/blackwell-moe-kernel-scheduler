"""V5: Blackwell CLC work stealing over the V1 math pipeline."""
...
def build_kernel(spec: TIRxMoESpec, plan: MoEWorkloadPlan):
    """Build a one-CTA CLC kernel whose launch coordinates are work IDs.

    The grid contains one CTA coordinate per ExpertTile.  A resident CTA starts
    with its own coordinate and asynchronously cancels a not-yet-launched CTA,
    inheriting that coordinate as its next work item.  CLC therefore replaces
    the global atomic queue without changing the TMA/tcgen05/TMEM pipeline.
    """
    ...
    def kernel(
        ...
        T.device_entry()
        # CLC needs a launch coordinate for every task.  Pending coordinates
        # are the pool from which already-resident CTAs steal.
        bx = T.cta_id([TILE_COUNT])
        ...
        # One CTA contributes four finished-slot arrivals per generation:
        # scheduler, loader, MMA and writeback.
        clc_sched = ClusterLaunchControlScheduler(
            pool,
            num_m_tiles=TILE_COUNT,
            num_n_tiles=1,
            l2_group_size=8,
            cta_group=1,
            finish_arrivals=4,
        ...
        if wg_id == 1:
            if warp_id == 3:
                ld = clc_sched.worker("clc_loader")
                ld.init(bx)
                tma_ps = PipelineState(PIPE_DEPTH, phase=1)
                if T.filter(lane_id, T.ptx.elect_sync()):
                    while ld.valid():
                        tile_id: T.int32 = ld.m_idx
                        ...
                        for k_tile in range(K_TILES):
                            ...
                            tma_ps.advance()
                        # Request completion is overlapped with the current
                        # tile's TMA mainloop; now adopt the stolen coordinate.
                        ld.consume()
                        ld.advance_coords()
                        ld.mark_done_if_drained()

            elif warp_id == 2:
                # cta_group=1 has cluster-local rank cbx=0.
                clc_sched.run_scheduler(0)

            elif warp_id == 0:
                mm = clc_sched.worker("clc_mma")
                mm.reset()
                mma_ps = PipelineState(PIPE_DEPTH, phase=0)
                ld_ps = PipelineState(1, phase=1)
                if T.filter(lane_id, T.ptx.elect_sync()):
                    while mm.valid():
                        # The MMA role consumes the generation count but not
                        # its coordinates: it computes whatever loader staged.
                        mm.consume()
            ...
            wb = clc_sched.worker("clc_writeback")
            wb.init(bx)
            wb_ps = PipelineState(1, phase=0)
            reg_bf16 = T.alloc_local((BLK_N,), d_type)
            while wb.valid():
                # Preserve the current coordinate before consuming the CLC
                # response for the following iteration.
                tile_id: T.int32 = wb.m_idx
